...
def comparison_players_blocks_ratio(player1, player2, move_time, m ,n ,rat, times):
    ...
    for x in range(times):
        ...
        game = NotAnimatedGameTest(map[0], map[1], map[2], player_1=contest_player, player_2=heavy_player,
...
def comparison_players_blocks_ratio_tests(player1, player2, move_time, m ,n ,rat, num_of_maps_same_ratio, tests_per_map):
    ties, win_1, win_2 = 0, 0, 0
...
def test_2_players_different_ratios(player1, player2, move_time, m ,n ,num_of_maps_same_ratio, tests_per_map):
    ...
    for x in range(1, 9, 1):
        rat = x/10
        rats.append(rat)
        ret = comparison_players_blocks_ratio_tests(player1, player2, move_time, m ,n ,rat, num_of_maps_same_ratio,tests_per_map)
...
def get_random_simetric_map_with_border(m, n, p):
    ...
    if n % 2 == 0:
        ...
        l = random.choices(['obs', 'free'], [num_of_obs, int((m * n -2*m)/2 - num_of_obs)], k=int((m * n -2*m)/2) )
        num = num_of_obsF(l)
        while (num < num_of_obs):
